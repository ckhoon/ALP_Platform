...
router.post('/', function(req, res){
	console.log("del ble blind - ");
	req.on('data', function(data) {
		var reqDev = JSON.parse(data);
		var status = -1;
		console.log(reqDev);
		for (let dev of req.app.devices.bleBlinds){
			if (dev.id == reqDev.id)
			{
				...
				var index = req.app.devices.bleBlinds.indexOf(dev);
				console.log(index);
				if (index != -1){
					status = index;
					req.app.devices.bleBlinds.splice(index, 1);
